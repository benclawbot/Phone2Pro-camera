...
EXPECTED={
 ...
 'candidateMethodCount':679,'classNodeCount':40,'classEdgeCount':35,
 'packageNodeCount':19,'packageEdgeCount':19,'syntheticCallbackEdgeCount':15274,
...
def load(root:pathlib.Path=pathlib.Path('.'))->dict[str,Any]:
 base=root/BASE; index=json.loads((base/'index.v1.json').read_text()); parts=index['parts']
 nodes=json.loads((base/parts['classNodes']).read_text())['nodes']; edges=json.loads((base/parts['classEdges']).read_text())['edges']; packages=json.loads((base/parts['packageGraph']).read_text()); methods=json.loads((base/parts['methods']).read_text())['methods']
 index['classGraph']={'nodes':nodes,'edges':edges}; index['packageGraph']={'nodes':packages['nodes'],'edges':packages['edges']}; index['routeCandidateMethods']=methods
 return index
...
def validate(d:dict[str,Any],root:pathlib.Path=pathlib.Path('.'))->None:
 ...
 nodes=d['classGraph']['nodes']; edges=d['classGraph']['edges']; methods=d['routeCandidateMethods']; names=[n['name'] for n in nodes]
 if len(names)!=len(set(names)) or not REQUIRED_CLASSES.issubset(names):raise ValueError('required class root missing')
 if len(nodes)!=40 or len(edges)!=35 or len(methods)!=12:raise ValueError('bounded graph size drift')
...
def main()->int:
 root=pathlib.Path(__file__).resolve().parents[1]; validate(load(root),root); print('Nothing Camera class graph is valid'); return 0
